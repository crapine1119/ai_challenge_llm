# Vendored from https://github.com/Tanat05/korcen (Apache-2.0).
# Local modifications:
#   - export 함수 정리
#   - guardrail service와 연동
# See third_party/korcen-LICENSE.txt (Apache-2.0)
...
try:
    ...
    print(
        f"{colorama.Fore.YELLOW}korcen: 'better_profanity' library not found. English filtering will be skipped.{colorama.Style.RESET_ALL}"
    )
    BETTER_PROFANITY_LOADED = False
except Exception as e:
    print(
        f"{colorama.Fore.RED}korcen: Could not load better_profanity censor words. English filtering might be affected. Error: {e}{colorama.Style.RESET_ALL}"
    )
    BETTER_PROFANITY_LOADED = False
...
def get_false_positive_regex(level: str):
    if level == "general":
        return FP_REGEX_GENERAL
    if level == "minor":
        return FP_REGEX_MINOR
    if level == "sexual":
        return FP_REGEX_SEXUAL
    if level == "belittle":
        return FP_REGEX_BELITTLE
    if level == "race":
        return FP_REGEX_RACE
    if level == "parent":
        return FP_REGEX_PARENT
    if level == "politics":
        return FP_REGEX_POLITICS
    if level == "english":
        return FP_REGEX_ENGLISH
    return None


def get_profanity_regex(level: str):
    if level == "general":
        return P_REGEX_GENERAL
    if level == "minor":
        return P_REGEX_MINOR
    if level == "sexual":
        return P_REGEX_SEXUAL
    if level == "belittle":
        return P_REGEX_BELITTLE
    if level == "race":
        return P_REGEX_RACE
    if level == "parent":
        return P_REGEX_PARENT
    if level == "japanese":
        return P_REGEX_JAPANESE
    if level == "chinese":
        return P_REGEX_CHINESE
    if level == "special":
        return P_REGEX_SPECIAL
    if level == "politics":
        return P_REGEX_POLITICS
    return None
        ...
        with open(filepath, "r", encoding="utf-8") as f:
            for line in f:
                ...
                try:
                    ...
                    print(
                        f"{colorama.Fore.RED}korcen: Error compiling custom regex '{pattern}' from {filepath}: {e}{colorama.Style.RESET_ALL}"
                    )
...
def create_filter_function(level_name: str):
    def filter_func(text: str, id: int = None) -> bool:
        if id is not None:
            if cache_size < 2:
                print(
                    f"{colorama.Fore.YELLOW}korcen: Cache size must be at least 2 for ID based filtering ({level_name}).{colorama.Style.RESET_ALL}"
                )
                return False
...
def highlight_profanity(text: str, id: int = None, level: str = "general", highlight_char: str = "!") -> str:
    if id is not None:
        if cache_size < 2:
            print(
                f"{colorama.Fore.YELLOW}korcen: Cache size must be at least 2 for ID based filtering (highlighting {level}).{colorama.Style.RESET_ALL}"
            )
            return text

    if level.lower() == "all":
        levels = ["general", "minor", "sexual", "belittle", "race", "parent", "special", "politics"]
    ...
    try:
        ...
        highlighted_text = flexible_regex.sub(
            lambda m: f"{highlight_char}{m.group(0)}{highlight_char}", text_to_highlight_on
        )
    except re.error as e:
        print(
            f"{colorama.Fore.RED}korcen: Error compiling flexible regex for '{matched_pattern}' (level {level}): {e}{colorama.Style.RESET_ALL}"
        )
        return text
...
def set_custom_filter_paths(include_path: str = CUSTOM_INCLUDE_FILE, exclude_path: str = CUSTOM_EXCLUDE_FILE):
    ...
    if CUSTOM_INCLUDE_REGEXES:
        print(
            f"{colorama.Fore.GREEN}korcen: Loaded {len(CUSTOM_INCLUDE_REGEXES)} custom include patterns from {include_path}{colorama.Style.RESET_ALL}"
        )
    CUSTOM_EXCLUDE_REGEXES = load_and_compile_custom_patterns(exclude_path)
    if CUSTOM_EXCLUDE_REGEXES:
        print(
            f"{colorama.Fore.GREEN}korcen: Loaded {len(CUSTOM_EXCLUDE_REGEXES)} custom exclude patterns from {exclude_path}{colorama.Style.RESET_ALL}"
        )
...
def check(text: str, id: int = None, foreign: bool = False) -> bool:
    ...
    if politics(text, id):
        return True
    if foreign:
        if BETTER_PROFANITY_LOADED and english(text, id):
            return True
        if japanese(text, id):
            return True
        if chinese(text, id):
...
if __name__ == "__main__":
    ...
    print(
        f"'http://example.com/sex 섹스하자' (sexual with URL): {highlight_profanity('http://example.com/sex 섹스하자', level='sexual')}"
    )
